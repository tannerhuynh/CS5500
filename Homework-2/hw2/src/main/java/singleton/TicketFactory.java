...
public class TicketFactory {
  ...
  public static ITicket create(JSONObject ticketInfo) {
    ...
    Counter.countLocation(departure);
    Counter.countLocation(arrival);

    switch (ticketType) {
      case "BUS":
        Counter.busCounter();
        return BusTicketSingleton.createSingleton(passenger, routeID, departDate, arrivalDate, departure,
                arrival,
                null);
      case "TRAIN":
        Counter.trainCounter();
        return TrainTicketSingleton.createSingleton(passenger, routeID, departDate, arrivalDate,
                departure,
                arrival,
                classType);
      case "PLANE":
        Counter.planeCounter();
        return PlaneTicketSingleton.createSingleton(passenger, routeID, departDate, arrivalDate, departure,
                arrival,
                classType);
      default:
